...
export default function Marquee() {
  ...
  return (
    <section className="border-y border-white/10 bg-ink-deep py-10">
      <p className="mb-7 text-center text-[10px] uppercase tracking-[0.4em] text-white/40">
      ...
      <div className="relative overflow-hidden no-scrollbar">
        <div className="flex w-max animate-marquee items-center gap-14 pr-14">
          {items.map((item, i) => (
            <span
              key={`${item}-${i}`}
              className="flex shrink-0 items-center gap-3 font-display text-xl uppercase tracking-tight text-white/35"
            >
              {item}
